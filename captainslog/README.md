...
- **Symlink-aware viewer lookup.** `--with-viewer` looks for `captainslog-viewer.py` next to this script. If `captainslog.sh` is invoked through a symlink (e.g. one on your `PATH` pointing back into this repo), the symlink is resolved to its real target first, so the viewer is found next to the actual file rather than next to the link.
